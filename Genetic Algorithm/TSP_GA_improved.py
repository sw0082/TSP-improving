    ...
    """for i in range(len(path) - 2):
        for j in range(len(path) - 1):
            if is_cross_pt(int(coord_list[path[i], 0]), int(coord_list[path[i], 1]),
                           ...
                           int(coord_list[path[i + 1], 1]), int(coord_list[path[j], 0]),
                           int(coord_list[path[j], 1]),
                           int(coord_list[path[j + 1], 0]), int(coord_list[path[j + 1], 1])):
                if (i == j):
                    continue
...
def selection(pool_cost, k):
    ...
    for i in range(0, K):
        ...
        for j in range(0, len(pool_cost)):
            if (sumOfrand < rand) and (rand <= (sumOfrand + float(prob[j]))):
                selected_indices[i] = j
                sumOfrand = 0
                break
            else:
                sumOfrand = sumOfrand + prob[j]
